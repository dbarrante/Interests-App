...
function load(src, state) {
  const factory = new Function(
    "imported", "saved", "_tagPickScope", "_tagPickIdx", "_tagPickId", "tabs", "AI_TAB_TAG",
    fn(src, "allTags") + "\n" + fn(src, "_tagPickItem") + "\nreturn { allTags, _tagPickItem };"
  );
  return factory(state.imported, state.saved, state.scope, state.idx, state.id, state.tabs || [], "__ai_research__");
}
