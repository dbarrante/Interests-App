...
const BASE = { cards: 10, saved: 3, tombstones: 2, maxCardUpdatedAt: 111, maxSavedUpdatedAt: 222, maxTombDeletedAt: 333, settingsUpdatedAt: 444, mutationRevision: 7 };
...
for (const [label, m] of [["core", require("../core/merge.js")], ["pwa", require("../pwa/merge.js")]]) {
  ...
  run(label + ": exports contentSignature; deterministic", () => {
    ...
    assert.ok(/^v2\|/.test(sig(BASE)), "versioned prefix so future field changes can never alias old signatures");
  });
...
run("signatureAggregates: empty store → all zeros; each mutation moves its aggregate", () => {
  ...
  assert.deepStrictEqual(a0, { cards: 0, saved: 0, tombstones: 0, maxCardUpdatedAt: 0, maxSavedUpdatedAt: 0, maxTombDeletedAt: 0, settingsUpdatedAt: 0, mutationRevision: 0 });
  ...
  assert.ok(a1.mutationRevision > a0.mutationRevision, "card upsert advances durable mutation revision");
  ...
  assert.ok(a2.mutationRevision > a1.mutationRevision, "tombstone advances durable mutation revision");
