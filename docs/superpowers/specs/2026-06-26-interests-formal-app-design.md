...
- **YouTube saving — still OPEN (2026-06-27):** in the same test, a YouTube save produced **no `[drainCaptures]` line at all** → nothing delivered. YouTube has no content script, so saving relies on right-click → "Save to Interests" (`clipCurrentPage`, sets `clip:true`). v2: get YouTube-specific repro (right-click action + extension badge/toast + whether the app was open) and trace why the clip isn't delivered/drained.
