...
- **Saved count not live (reported 2026-06-27):** saving a clip doesn't increment the Saved count/tab until the app is reloaded. Almost certainly the SAME post-change live-refresh root cause as the import/restore item above — fix together (re-render after `Store.putSaved`).
