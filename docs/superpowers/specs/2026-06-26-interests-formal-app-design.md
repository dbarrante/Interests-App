...
- **YouTube & Pinterest captures don't save (reported 2026-06-27):** saves from YouTube and Pinterest don't land in the library, while **Stumble works** (so the core save→store path is OK). Investigate the per-platform path in v2: the extension's `capture-configs.js`/capture engine for those sites, the clip → `POST /api/captures` → `Store.putSaved` flow, and platform URL handling. Likely an extension/platform-specific bug, not a store bug.
