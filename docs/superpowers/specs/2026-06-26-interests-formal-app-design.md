...
- ✅ **FIXED 2026-06-27 (commit 17c7ef7) — Post-import live refresh:** the Import-legacy handler wrote the DB but never reloaded in-memory state, so records only appeared after a restart. Now reloads the renderer on import success (restore-from-backup already did this). Store-move needs no refresh — the records are unchanged, only relocated.
- ✅ **FIXED 2026-06-27 (commit 17c7ef7) — Saved/imported counts live:** `saveItem`/`unsaveItem` persisted but never called `updateCounts()`; `persistAll()` now refreshes the count badges centrally, covering every save/unsave path.
- **Pinterest saving — VERIFIED WORKING (console evidence 2026-06-27):** clips are delivered (`POST /api/captures`) and drained correctly. Routing: a pin already in the imported library → `[clip] filled imported card from save` (enriches that card, no duplicate, by design); a genuinely new pin → `[clip] received clip` → `addClip` → new Saved card (count updates via `addClip`'s `persistAll();updateCounts()`). The "isn't saving" perception came from re-saving already-imported pins (dedup-into-imported). Not a bug. (Possible v2 polish: a toast/indication when a save enriches an imported card so it doesn't feel like a no-op.)
- **Right-click "Save to Interests" on an image — FIXED 2026-06-27 (commit 58bb8aa):** now saves the clicked image + its link (`info.srcUrl`/`linkUrl`, `noShot`) instead of a full-page screenshot. Verified: right-clicking a YouTube thumbnail delivered `[clip] received clip for youtube.com/shorts/...`; Pinterest pins too.
- **YouTube watch-page video capture (2026-06-27):** right-click does NOT work on the *playing* video — YouTube hijacks the player's context menu, so our item never appears. Workaround that works today: the extension **popup → "📎 Clip this page to Interests"** (grabs the watch page's og:image/thumbnail + title). v2 polish: (a) on YouTube, prefer the clean og:image/maxres thumbnail over the page screenshot in `addClip`; (b) optionally inject an in-page "Save to Interests" button on watch pages (like the Pinterest Save-mirror).
- **Saved-pin image reliability (2026-06-27):** a saved pin showed `image.thum.io/...pin/...→403` — the stored "image" was a pin *permalink* (not an image), so the render fallback proxied it and 403'd. v2: when saving a pin, store a stable image (the pin's `i.pinimg.com` src or the permalink's og:image), and improve the image fallback when a proxy 403s.
- **NEW v2 feature (requested 2026-06-27): Pinterest native Save-button → library import.** When the user clicks Pinterest's OWN Save button, mirror it into the app as a saved/library item (the in-page `capture-core` Save-mirror). Make this reliable and land pins in the Saved library, importing into the app automatically.
- **NEW v3 idea (requested 2026-06-28): YouTube playlist-save → library import.** When the user adds a video to a YouTube playlist (Watch Later / Liked / a custom list), also capture that video into the Saved library — the YouTube analog of the Pinterest native-Save-button feature above. Two implementation paths: **(A, recommended primary) real-time Save-mirror** — extend the YouTube `capture-config` with a `saveTrigger` that watches the "Save to playlist" dialog (`ytd-add-to-playlist-renderer` / playlist-option checkboxes); on a checkbox toggle, capture the current `watch?v=…` + `i.ytimg` thumbnail and route to Saved. No OAuth/quota; in-browser only. **(B) periodic playlist read** — fold into the *scheduled-extraction* sub-project: read a playlist on a bounded/stoppable schedule via the YouTube Data API (OAuth) or scrape; catches saves from any device incl. phone, but heavier and subject to the hard "nothing unbounded/beyond my control" rule. **Open design question (decide at build time):** fire on **every** playlist save vs. only a designated trigger playlist (e.g. Watch Later or a dedicated "→ Interests" list) to avoid flooding the library.

- **Clip mis-attribution / wrong-card image overwrite — DATA SAFETY (reported 2026-06-27):** a popup "Clip this page" of a YouTube video was routed to `[clip] filled imported card from save: "3d printed Super-girl…"` (an unrelated Pinterest pin) and overwrote that card's image with the YouTube capture. The clip→imported-card matching (`drainCaptures`, ~web/index.html:3525-3540) is too loose: exact/normalized-URL match is fine, but the recently-opened "active card" fallback (and any empty-`domain()` comparison) can attach a clip to the wrong card and `setCardImage` over it. v2 fix: (a) only fill an imported card from a clip on a **high-confidence same-domain URL match**; require `domain()` to be non-empty on both sides; (b) a **popup "Clip this page" should always create a NEW saved clip**, never fill a recently-opened card; (c) never overwrite an existing card image on a low-confidence match. Add temporary diagnostic logging of `cap.url` + the chosen match path to pinpoint the exact trigger.
- **Capture & routing v2 pass:** treat the above capture items (right-click/popup/player behavior, YouTube watch-page quality, pin-image reliability, clip mis-attribution) as one coherent subsystem to redesign, not piecemeal patches.
