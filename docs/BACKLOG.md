...
## 2026-07-19 — Auto-import check-interval + Pinterest & Google-saves platforms (v1.12.30)
- **Configurable interval:** Settings "Check every" dropdown (1 day default / 12 / 8 / 4 / 2 / 1h), one global `S.autoImportEvery` → config `intervalHours` (clamped 1-24) → the extension re-creates its auto-check alarm within ~30s of a change (applied value cached in `chrome.storage.local`, no reload). "Check now" and the on/per-platform gates unchanged.
- **Two new platforms, same pipeline as FB/IG:** Pinterest (all your pins) + Google saved items. Pure parsers `extension/lib/saved-parse-{pin,gs}.js`; platform whitelist `fb|ig|pin|gs` with own ledgers/status records; `pin-auto`/`gs-auto` route `import-auto`; src `pinterest`/`google` (existing icons/pills). Settings gains both checkboxes + status rows; section renamed "Platform auto-import".
- **Capture-tuned facts (captures gitignored):** **Pinterest** — relative or absolute `/pin/<digits>` anchors merge on the numeric id, canonical `pinterest.com/pin/<id>/` urls, title chain aria (junk "Untitled pin page" demoted) → prefix-stripped img alt ("This may contain:"/"This contains an image of:") → inner text; `i.pinimg.com` passes raw (not signed CDN). Scrape lands on `/me/pins/` with TWO home-feed guards (pre-parse pathname + post-parse diag-href re-check) — recommendation shelves must never import as saves; live `/me/pins/` capture verified clean ("All Pins" profile page, no related-pin containers). **Google** — the bare `/save` page is the collections OVERVIEW (zero item links); scrape the flat `interests/saved/list/allsaves`; parser unwraps `google.<tld>/url?q=` redirect wrappers to the external target, merges the per-item thumbnail+title anchor pair on the normalized target url (platformKey — no stable item id; over-length keys hashed), accepts direct-external anchors only with card evidence (aria/img); accounts/consent walls → login-required. Live replays: Pinterest 17/17 + 100/100, Google 86/86, all titled.
- **Data-safety review (2026-07-19):** append-only pipeline (no delete/mutate path). HIGH — gs test fixture had transcribed real capture strings; purged pre-push, fixtures now fully invented (.example). MEDIUM — pin home-feed guard hardened with the post-parse re-check. LOW — over-length gs keys hashed not dropped. LOW (hash-routed SPA url collision) documented-accepted. Spec `docs/superpowers/specs/2026-07-19-autoimport-interval-pinterest-google-design.md`, plan `docs/superpowers/plans/2026-07-19-autoimport-interval-pinterest-google.md`.

## 2026-07-18 — Facebook/Instagram auto-import (built across 4 tasks; pending version bump/release)
- **Saves made natively on Facebook/Instagram (any device) now reach the app without a manual export/import.** The desktop browser extension checks each platform's Saved page once a day (plus an app-side "Check now"), in one inactive tab, using the user's own logged-in session; survivors land in the **Imported** tab as taste signals — never Saved, never auto-deleted from the platform side. Ships **OFF by default** (`autoImportOn`); "Check now" always works regardless of the master switch.
- **Extension (`extension/background.js` + `extension/lib/saved-parse-{fb,ig}.js`):** a daily `chrome.alarms` alarm (respecting the `autoImportOn`/per-platform config gate) plus a 30s mailbox poll for the app's "Check now" (always runs, bypassing the gate) — `runAutoImportCheck`/`pollAutoImportRequest`. One page visit per platform per run, sequential, scrolled ~2 screens (~100-entry cap), tab always closed in a `finally`. A login wall or zero-entry parse reports a status with **zero items** — scrapers fail soft, never partial garbage. Signed-CDN images are converted through the existing `durableImage()` before delivery.
- **Core (`core/autoimport.js` + routes in `core/server.js`):** `POST /api/auto-import` validates/caps the untrusted batch (body ≤1MB, ≤200 items, per-field caps incl. an oversized-image STRIP-not-reject amendment), dedupes against a permanent per-platform platformKey ledger (kv `ia_autoimport_seen_fb`/`_ig`, null-prototype safe, capped/pruned at 5000) **and** a same-URL-in-library guard, then appends survivors to the same `ia_capture_queue` mailbox `/api/captures` feeds — tagged `source: "fb-auto"|"ig-auto"`, deliberately **no `clip:true`**. A platformKey once seen stays ledgered forever, so deleting a card in the app permanently blocks that post from re-importing even if it's still saved on the platform. `GET /api/auto-import/config` (extension poll gate), `POST/GET /api/auto-import/request` (Check-now mailbox, mirrors `/api/capture-request`), `GET /api/auto-import/status` (last-run `ia_autoimport_last_fb`/`_ig` records).
- **Renderer routing (the linchpin — Task 4):** `web/route-capture.js`/`pwa/route-capture.js` gained a pure `{action:"import-auto"}` decision, resolved **before every other branch** (clip/recap/id-match/active-card) so an auto-import capture can never false-stamp an open or matching card's image instead of becoming its own Imported entry. `web/index.html`/`pwa/index.html`'s `drainCaptures` routes that action through the SAME `dedupeImported`/`ingestImported` machinery file/bookmark import uses (`autoImportItemFromCap` helper) — new or enriched Imported card, `src:"facebook"|"instagram"`, `desc:"Saved from Facebook/Instagram"`, imageless is fine by design (the existing FB/IG "Get pictures" flow backfills it).
- **Settings (desktop-only, `secAutoImport`):** master toggle (`autoImportOn`, default **false**) + per-platform Facebook/Instagram checkboxes (`autoImportFb`/`autoImportIg`, default true) persisted through the normal `save("settings",S)` pipeline (same `ia_settings` kv key the extension's config poll reads — no separate config endpoint needed); "Check now" button; per-platform last-checked status line. Hidden on the PWA via the existing `window.IA_IDB` desktop-only hide list (no local server there to back it) — new `Store.setAutoImportRequest`/`Store.getAutoImportStatus` methods added to `web/storage.js` only.
- **Tests:** `tests/autoimport-core.test.js`, `tests/autoimport-endpoint.test.js`, `tests/autoimport-fb-parse.test.js`, `tests/autoimport-ig-parse.test.js`, `tests/autoimport-ext-wiring.test.js` (Tasks 1-3), extended `tests/route-capture.test.js` (precedence cases incl. the binding task-3 carry-forward: an `-auto` capture with a matching open active card still routes `import-auto`) + new `tests/autoimport-ui-wiring.test.js` (Task 4, source-scan). Spec: `docs/superpowers/specs/2026-07-17-platform-auto-import-design.md`, plan `docs/superpowers/plans/2026-07-18-platform-auto-import.md`.
- **Live-tuning DONE 2026-07-19 (v1.12.28)** — auto-import verified end-to-end on both platforms against the user's real logged-in pages. Traps found live, all regression-tested: (1) **`instagram.com/saved/` is the PROFILE of a real account named @saved** — the IG scrape now lands on HOME, discovers the viewer's username from the nav avatar, then opens `/<username>/saved/all-posts/`; username-prefixed profile-grid links (`/<user>/p/…`) can never match. (2) Lazy grids mount slowly/partially in a HIDDEN tab (IG ~10 tiles, FB ~13) — the collector polls scroll→settle(1s)→parse; per-check catch window is the newest ~10-13 items (backlogs were imported one-time from full visible-tab captures). (3) Full-res converted images blew the core's 1MB delivery cap (413 swallowed silently) — delivery now uses `durableThumb` (640px JPEG) under an 850KB batch budget. (4) The FB /saved/ DOM includes the notification bell's pre-rendered dropdown — notification-marked urls (`notif_id`/`notif_t`/`ref=notif`) are rejected before pattern-matching. Parsers merge ALL anchors per bare post id (FB renders ~3 anchors/card: thumbnail/excerpt/byline; excerpt wins, durations demoted; group permalink+posts shapes collapse). Diag breadcrumbs (`auto-import <p> diag:` in the extension SW console) are permanent. Daily auto-check enabled 2026-07-19.
- **Deferred → next session candidate:** remove the ~10-13 per-check window by reading the platforms' own paginated saved-feed API from page context (IG: `/api/v1/feed/saved/posts/`) with DOM-scrape fallback — needed only if saving pace exceeds ~10/day between checks.

## Investigate 2026-07-16 (from live sync debugging)

- [x] **RESOLVED 2026-07-17** — root cause was NOT serialization: killed test runs had poisoned the production `%APPDATA%` config pointer, and the phantom re-stamps came from the PWA's unconditional `nowStamp` on full-array persists after an iPhone storage reset. Full forensics + fixes + recovery: `docs/superpowers/2026-07-17-incident-root-cause.md`. Remaining hardening items listed there (boot-time temp-store guard, backupDir sanity check, merge image-copy from any holder incl. self).
