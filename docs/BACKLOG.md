...
- **Deferred:** the FB/IG scraper selectors are best-effort until a live-tuning round against the real Saved pages (manual step — reload the extension, run "Check now" with DevTools open). PWA SHELL_CACHE bumped (v29→v30) for the pwa/index.html + pwa/route-capture.js markup/script changes even though the feature itself is desktop-only.
